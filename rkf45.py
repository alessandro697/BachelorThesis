...
"""Translation of fortran 77 rkf45 to python via matlab
   ...
   Nick corrected the vector stuff below adding asarray() and .tolist() 
   as needed.   			By Nick Manini, 17 May 2016
"""
from numpy import *
...
def r8_fehl ( f, neqn, y, t, h, yp ):
    ...
    """
    ch = h / 4.0
    f5 = (asarray(y) + ch * asarray(yp)).tolist()
    f1 = f( t + ch, f5 )
    ch = 3.0 * h / 32.0
    f5 = (asarray(y) + ch * ( asarray(yp) + 3.0 * asarray(f1) )).tolist()
    f2 = f( t + 3.0 * h / 8.0, f5 )
    ch = h / 2197.0
    f5 = (asarray(y) + ch * ( 1932.0 * asarray(yp) + ( 7296.0 * asarray(f2) - 7200.0 * asarray(f1) ) ) ).tolist()
    f3 = f( t + 12.0 * h / 13.0, f5 )
    ch = h / 4104.0
    f5 = (asarray(y) + ch * (( 8341.0 * asarray(yp) - 845.0 * asarray(f3)) + ( 29440.0 * asarray(f2) - 32832.0 * asarray(f1) ) ) ).tolist()
    f4 = f( t + h, f5 )
    ch = h / 20520.0
    f1 = (asarray(y) + ch * ( ( -6080.0 * asarray(yp) + ( 9295.0 * asarray(f3) - 5643.0 * asarray(f4) ) ) + \
                 ( 41040.0 * asarray(f1) - 28352.0 * asarray(f2) ) ) ).tolist()
    ...
    #
    ch = h / 7618050.0
    s = (asarray(y) + ch * ( ( 902880.0 * asarray(yp) + ( 3855735.0 * asarray(f3) - 1371249.0 * asarray(f4) ) ) \
        + ( 3953664.0 * asarray(f2) + 277020.0 * asarray(f5) ) ) ).tolist()
